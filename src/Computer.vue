<template>
  <div>
    <el-alert type="info" title="说明" description>第一行是当前角色的状态，第二行是目标状态，后面是已经有的资源。框里的数字可以自己输入具体的数字</el-alert>
    <div>
      <el-form :inline="!short" :model="char" :rules="rules">
        ...
        <div class="my-form-wrapper extra-exp">
          <div v-for="(card, index, key) in exp_cards" :key="card.iconId">
            <el-form-item label="已有经验/千" class="my-form-item-wrapper">
              <div slot="label">
...
import { exp_cards } from './components/utils';
Vue.use(Form);
Vue.use(FormItem);
Vue.use(Select);
Vue.use(Option);
  ...
  computed: {
    ...
    CE5() {
      const result = Math.ceil(
        (this.MoneyNeed - this.char.base.money * 1000) / 7500
      );
  ...
  methods: {
    calBase(target) {
      ...
      for (let i = this.char.phases_now; i < phases + 1; i++) {
        for (
          let j = this.char.level_now - 1;
          j < this.data.maxLevel[star][i];
          j++
        ) {
