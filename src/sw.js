...
workbox.routing.registerRoute(/https:\/\/andata.somedata.top\/dataX\/(.+)(?<!\.mp3)$/, new workbox.strategies.StaleWhileRevalidate({ 'cacheName': 'oss-cache', plugins: [new workbox.cacheableResponse.Plugin({ statuses: [0, 200] }), new workbox.broadcastUpdate.Plugin({ channelName: 'oss-update' })] }), 'GET');
...
workbox.routing.registerRoute(/https:\/\/somedata.top\/Arknights/, new workbox.strategies.StaleWhileRevalidate(), 'GET');
